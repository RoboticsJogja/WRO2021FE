...
At its core, the HC-SR04 Ultrasonic distance sensor consists of two ultrasonic transducers. The one acts as a transmitter which converts electrical signal into 40 KHz ultrasonic sound pulses. The receiver listens for the transmitted pulses. If it receives them it produces an output pulse whose width can be used to determine the distance the pulse travelled. The sensor is small, easy to use, and offers excellent non-contact range detection between 2 cm to 400 cm with an accuracy of 3mm. Since it operates on 5 volts, it can be hooked directly to any 5V logic microcontrollers. The sides of the two ultrasonic sensors are protected by a collision-protecting structure, which are also equipped by roller wheels in order to avoid direct collisions and roll on the sides of the walls instead.
